...
#define MAX_HEATINGTIME_NO_TEMP_CHANGE_MINUTES 15   /*detect when temp sensor is not in the water and prevent overheating*/
...
// HACK UNTIL WE ADD BUTTONS:
//#define TARGET_TEMP 52   /*TEST*/
#define TARGET_TEMP 60   /*Medium rare steak*/
//#define TARGET_TEMP 85   /*Most root veg*/
...
void setup() {
  ...
  */
  ...
  targetTemp = min(TARGET_TEMP, MAX_TARGET_TEMP);    
...
void loop() {   
  ...
  case INITIAL_WAIT:      
    lcd.setCursor(0,1);
    lcd.print("INIT: wait for stability");
  ...
  case TEMP_DROP:
    ...
    if (waitForSuddenRise == false && IsStabilizing())
    {
      if (abs(actualTemp - environmentTemp) < abs(actualTemp - tempBeforeDrop))
      {
        ...
        lcd.setCursor(0,1);
        lcd.print("TEMP_DROP : wait temprise");
      } else {
        ...
        if (doBackToFirstRampWhenStabilizing)
        {
          ...
          lcd.setCursor(0,1);
          lcd.print("TEMP_RISE : initial ramping");
          opState = FIRST_RAMP;
        } 
        else 
        {       
  ...
  case TEMP_RISE:
    ...
    {     
      if (doBackToFirstRampWhenStabilizing)
      {
        ...
        lcd.setCursor(0,1);
        lcd.print(" TEMP_RISE : back to initial ramping");
        opState = FIRST_RAMP;
      } 
      else 
      { 
        ...
        lcd.setCursor(0,1);
        lcd.print(" TEMP_RISE : back to normal : reg");
        EnterRegulateStateOrWaitSmoothLowering();
      }
    }
    WatchForTempFalling();
    ...
    {   
      ...
      else 
      {
        if ( IsStabilizingOrDropping() )
        {
          ...
          lcd.setCursor(0,1);
          lcd.print(" COUNTER_FALL finished : reg");
          //reset counter
          warningsBeforeCounterFall = 3;
          EnterRegulateStateOrWaitSmoothLowering();
        }
    ...
    { 
      ...
      if ( ((long) (millis() - tCheckStabilize) >= 0) && isCounteracting )
      {
        ...
        { 
          ...
          lcd.setCursor(0,1);
          lcd.print("NATURAL_DROP ended: wait stabilize");
...
void EnterRegulateStateOrWaitSmoothLowering()
{
  ...
  if (actualTemp > 0 & actualTemp < targetTemp + 0.3)
  {
    Serial.println("EnterRegulateState !");
    lcd.setCursor(0,1);
    lcd.print("EnterRegulateState !");
...
void setupCutOffTempForInitialRamping()
{
  // calculate turn-off temperature
   ...
   String firstRampString = "1RampCutOff:";
   String firstRamp = firstRampString + firstRampCutOffTemp;
   Serial.println(firstRamp);
...
void PerformFirstRamp()
{
    ...
    // switch off heat and wait for stabilization
       if (isRelayOn()) {
         ...
         String message = stopString + actualTemp;
      ...
      // heat fullsteam ahead
       ...
       burnupTime = tOperationalDelay / 20; // arbitrary... to be perfected
           ...
           String msgString = "opDelay:";
           String msg = msgString + tOperationalDelay;
...
boolean isRelayOn()
{
  return digitalRead(RELAY_OUT_PIN) == LOW;
}

boolean isRelayOff()
{
  return digitalRead(RELAY_OUT_PIN) == HIGH;
}
